...
class TestCpplint(CpplintTestBase):
    ...
    def GetNamespaceResults(self, state: _CppLintState, lines):
        ...
        for i in range(lines.NumLines()):
            ...
            CheckForNamespaceIndentation(state, 'foo.h', nesting_state,
                                         lines, i, error_collector)

        return error_collector.Results()

    def testForwardDeclarationNameSpaceIndentation(self, state: _CppLintState):
    ...
    def testNameSpaceIndentationForClass(self, state: _CppLintState):
        lines = ['namespace Test {',
                 'void foo() { }',
                 '  class Test {',
                 '  };',
    ...
    def testIncludeWhatYouUseNoImplementationFiles(self, state: _CppLintState):
    ...
    def testIncludeWhatYouUse(self, state: _CppLintState):
        self.TestIncludeWhatYouUse(state,
                                   """#include <vector>
           std::vector<int> foo;
        """,
                                   '')
    ...
    def testCheckDeprecated(self, state: _CppLintState):
    ...
    def testCheckPosixThreading(self, state):
        ...
        self.TestSingleLineLint(state, 'var = rand()',
                                'Consider using rand_r(...) instead of rand(...)'
                                ' for improved thread safety.'
                                '  [runtime/threadsafe_fn] [2]')
        self.TestSingleLineLint(state, 'var = strtok(str, delim)',
    ...
    def testIllegalOperatorOverloading(self, state):
        ...
        self.TestSingleLineLint(state, 'void operator & (  ) { }',
    ...
    def testDisallowMacrosAtEnd(self, state):
        for macro_name in (
            ...
            cpplint.ProcessFileData(state,
                                    ...
                                    error_collector)
            assert (
                           '%s should be the last thing in the class' % macro_name) + '  [readability/constructors] [3]' == error_collector.Results()
    ...
    def testCheckCheck(self, state):
        self.TestSingleLineLint(state, 'CHECK(x == 42);',
        ...
        self.TestSingleLineLint(state, 'DCHECK(x == 42);',
        ...
        self.TestSingleLineLint(state, 'CHECK(4\'2 == x);',
                                'Consider using CHECK_EQ instead of CHECK(a == b)'
                                '  [readability/check] [2]')
    ...
    def testCheckAltTokens(self, state):
        self.TestSingleLineLint(state, 'true or true',
                                'Use operator || instead of or'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'true and true',
                                'Use operator && instead of and'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'if (not true)',
                                'Use operator ! instead of not'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, '1 bitor 1',
                                'Use operator | instead of bitor'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, '1 xor 1',
                                'Use operator ^ instead of xor'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, '1 bitand 1',
                                'Use operator & instead of bitand'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'x = compl 1',
                                'Use operator ~ instead of compl'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'x and_eq y',
                                'Use operator &= instead of and_eq'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'x or_eq y',
                                'Use operator |= instead of or_eq'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'x xor_eq y',
                                'Use operator ^= instead of xor_eq'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'x not_eq y',
                                'Use operator != instead of not_eq'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'line_continuation or',
                                'Use operator || instead of or'
                                '  [readability/alt_tokens] [2]')
        self.TestSingleLineLint(state, 'if(true and(parentheses',
                                'Use operator && instead of and'
                                '  [readability/alt_tokens] [2]')
    ...
    def testNonConstReference(self, state):
        ...
        self.TestSingleLineLint(state, 'bool operator>(Foo& s, Foo& f);',
        ...
        self.TestSingleLineLint(state, 'void foo(Bar*& p);',
                                operand_error_message % 'Bar*& p')
        self.TestSingleLineLint(state, 'void foo(const Bar*& p);',
                                operand_error_message % 'const Bar*& p')
        self.TestSingleLineLint(state, 'void foo(Bar const*& p);',
                                operand_error_message % 'Bar const*& p')
        self.TestSingleLineLint(state, 'void foo(struct Bar*& p);',
                                operand_error_message % 'struct Bar*& p')
        self.TestSingleLineLint(state, 'void foo(const struct Bar*& p);',
                                operand_error_message % 'const struct Bar*& p')
        self.TestSingleLineLint(state, 'void foo(struct Bar const*& p);',
                                operand_error_message % 'struct Bar const*& p')
        # Non-const reference to a templated type is not OK.
        self.TestSingleLineLint(state, 'void foo(std::vector<int>& p);',
        ...
        self.TestSingleLineLint(state, 'void Func(const vector <int> &const_x, '
    ...
    def testMismatchingSpacesInParens(self, state):
        self.TestSingleLineLint(state, 'if (foo ) {', 'Mismatching spaces inside () in if'
                                                      '  [whitespace/parens] [5]')
        self.TestSingleLineLint(state, 'switch ( foo) {', 'Mismatching spaces inside () in switch'
                                                          '  [whitespace/parens] [5]')
        self.TestSingleLineLint(state, 'for (foo; ba; bar ) {', 'Mismatching spaces inside () in for'
                                                                '  [whitespace/parens] [5]')
        self.TestSingleLineLint(state, 'for (; foo; bar) {', '')
        self.TestSingleLineLint(state, 'for ( ; foo; bar) {', '')
        self.TestSingleLineLint(state, 'for ( ; foo; bar ) {', '')
        self.TestSingleLineLint(state, 'for (foo; bar; ) {', '')
        self.TestSingleLineLint(state, 'while (  foo  ) {', 'Should have zero or one spaces inside'
    ...
    def testSpacingForFncall(self, state):
        ...
        self.TestSingleLineLint(state, 'for (foo;bar;baz) {', 'Missing space after ;'
        ...
        self.TestSingleLineLint(state, 'abc;// this is abc',
        ...
        self.TestSingleLineLint(state, 'foo( bar)', 'Extra space after ( in function call'
                                                    '  [whitespace/parens] [4]')
        ...
        self.TestSingleLineLint(state, 'foobar(     \\', '')
        self.TestSingleLineLint(state, '( a + b)', 'Extra space after ('
                                                   '  [whitespace/parens] [2]')
        self.TestSingleLineLint(state, '((a+b))', '')
        self.TestSingleLineLint(state, 'foo (foo)', 'Extra space before ( in function call'
                                                    '  [whitespace/parens] [4]')
    ...
    def testSpacingBeforeBraces(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'if (foo){', 'Missing space before {'
                                                    '  [whitespace/braces] [5]')
        self.TestSingleLineLint(state, 'for{', 'Missing space before {'
                                               '  [whitespace/braces] [5]')
        self.TestSingleLineLint(state, 'for {', '')
        self.TestSingleLineLint(state, 'EXPECT_DEBUG_DEATH({', '')
        self.TestSingleLineLint(state, 'std::is_convertible<A, B>{}', '')
        self.TestSingleLineLint(state, 'blah{32}', 'Missing space before {'
                                                   '  [whitespace/braces] [5]')
        ...
        self.TestSingleLineLint(state, 'constexpr int64_t kBatchGapMicros ='
                                       ' int64_t{7} * 24 * 3600 * 1000000;  // 1 wk.', '')
        self.TestSingleLineLint(state, 'MoveOnly(int i1, int i2) : ip1{new int{i1}}, '
                                       'ip2{new int{i2}} {}',
                                '')

    def testSemiColonAfterBraces(self, state):
        self.TestSingleLineLint(state, 'if (cond) { func(); };',
                                'You don\'t need a ; after a }  [readability/braces] [4]')
        self.TestSingleLineLint(state, 'void Func() {};',
                                'You don\'t need a ; after a }  [readability/braces] [4]')
        self.TestSingleLineLint(state, 'void Func() const {};',
                                'You don\'t need a ; after a }  [readability/braces] [4]')
        ...
        for keyword in ['struct', 'union']:
            for align in ['', ' alignas(16)']:
                for typename in ['', ' X']:
                    for identifier in ['', ' x']:
                        self.TestSingleLineLint(state, keyword + align + typename + ' {}' + identifier + ';',
                                                '')
        ...
        self.TestSingleLineLint(state, 'TEST(TestCase, TestName) {};',
                                'You don\'t need a ; after a }  [readability/braces] [4]')
        self.TestSingleLineLint(state, 'TEST_F(TestCase, TestName) {};',
                                'You don\'t need a ; after a }  [readability/braces] [4]')

        self.TestSingleLineLint(state, 'file_tocs_[i] = (FileToc) {a, b, c};', '')
        self.TestMultiLineLint(state, 'class X : public Y,\npublic Z {};', '')

    def testSpacingBeforeBrackets(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'int numbers [] = { 1, 2, 3 };',
    ...
    def testLambda(self, state):
        ...
        self.TestSingleLineLint(state, 'int operator[](int x) {};',
                                'You don\'t need a ; after a }  [readability/braces] [4]')
    ...
    def testBraceInitializerList(self, state):
        ...
        self.TestMultiLineLint(state, 'MyClass::MyClass()\n'
                                      ...
                                      '          Func()} {\n'
                                      '}\n', '')
        self.TestSingleLineLint(state, 'const pair<string, string> kCL' +
                                ('o' * 41) + 'gStr[] = {\n',
    ...
    def testSpacingAroundElse(self, state: _CppLintState):
        self.TestSingleLineLint(state, '}else {', 'Missing space before else'
                                                  '  [whitespace/braces] [5]')
        self.TestSingleLineLint(state, '} else{', 'Missing space before {'
                                                  '  [whitespace/braces] [5]')
        self.TestSingleLineLint(state, '} else {', '')
        self.TestSingleLineLint(state, '} else if (foo) {', '')

    def testSpacingWithInitializerLists(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'int v[1][3] = {{1, 2, 3}};', '')
        self.TestSingleLineLint(state, 'int v[1][1] = {{0}};', '')

    def testSpacingForBinaryOps(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'if (foo||bar) {', 'Missing spaces around ||'
                                                          '  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'if (foo<=bar) {', 'Missing spaces around <='
                                                          '  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'if (foo<bar) {', 'Missing spaces around <'
        ...
        self.TestSingleLineLint(state, 'if (foo>bar) {', 'Missing spaces around >'
                                                         '  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'if (foo<bar->baz) {', 'Missing spaces around <'
                                                              '  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'if (foo<bar->bar) {', 'Missing spaces around <'
                                                              '  [whitespace/operators] [3]')
        ...
        self.TestSingleLineLint(state, '10<<a',
                                'Missing spaces around <<  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'a<<20',
                                'Missing spaces around <<  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'a<<b',
                                'Missing spaces around <<  [whitespace/operators] [3]')
        ...
        self.TestSingleLineLint(state, 'a>>b',
                                'Missing spaces around >>  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, '10>>b',
                                'Missing spaces around >>  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'LOG(ERROR)<<*foo',
                                'Missing spaces around <<  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'LOG(ERROR)<<&foo',
                                'Missing spaces around <<  [whitespace/operators] [3]')
    ...
    def testSpacingBeforeLastSemicolon(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'call_function() ;',
        ...
        self.TestSingleLineLint(state, 'while (true) ;',
                                ...
                                'empty statement, use {} instead.'
                                '  [whitespace/semicolon] [5]')
        self.TestSingleLineLint(state, 'default:;',
    ...
    def testEmptyBlockBody(self, state):
        ...
        self.TestSingleLineLint(state, 'if (true);',
        ...
        self.TestSingleLineLint(state, 'if (test) {}',
    ...
    def testSpacingForRangeBasedFor(self, state: _CppLintState):
    ...
    def testStaticOrGlobalSTLStrings(self, state):
        ...
        self.TestSingleLineLint(state, 'string foo;',
                                nonconst_error_msg)
        self.TestSingleLineLint(state, 'string kFoo = "hello";  // English',
                                nonconst_error_msg)
        self.TestSingleLineLint(state, 'static string foo;',
                                nonconst_error_msg)
        self.TestSingleLineLint(state, 'static const string foo;',
                                error_msg % 'static const char foo')
        self.TestSingleLineLint(state, 'static const std::string foo;',
                                error_msg % 'static const char foo')
        self.TestSingleLineLint(state, 'string Foo::bar;',
                                nonconst_error_msg)

        self.TestSingleLineLint(state, 'std::string foo;',
                                nonconst_error_msg)
        self.TestSingleLineLint(state, 'std::string kFoo = "hello";  // English',
                                nonconst_error_msg)
        self.TestSingleLineLint(state, 'static std::string foo;',
                                nonconst_error_msg)
        self.TestSingleLineLint(state, 'static const std::string foo;',
                                error_msg % 'static const char foo')
        self.TestSingleLineLint(state, 'std::string Foo::bar;',
                                nonconst_error_msg)
        ...
        self.TestSingleLineLint(state, 'string const &FileInfo::Pathname() const;', '')
        self.TestSingleLineLint(state, 'string VeryLongNameFunctionSometimesEndsWith(\n'
                                       '    VeryLongNameType very_long_name_variable) {}', '')
        self.TestSingleLineLint(state, 'template<>\n'
        ...
        self.TestSingleLineLint(state, 'string Class<Type>::Method() const {\n'
                                       '  return "";\n'
                                       '}\n', '')
        self.TestSingleLineLint(state, 'string Class<Type>::Method(\n'
                                       '   int arg) const {\n'
                                       '  return "";\n'
                                       '}\n', '')
    ...
    def testToDoComments(self, state: _CppLintState):
        ...
        self.TestSingleLineLint(state, r'EXPECT_EQ("\\", '
                                       r'NormalizePath("/./../foo///bar/..//x/../..", ""));',
                                '')

    def testTwoSpacesBetweenCodeAndComments(self, state):
        self.TestSingleLineLint(state, '} // namespace foo',
                                'At least two spaces is best between code and comments'
                                '  [whitespace/comments] [2]')
        self.TestSingleLineLint(state, '}// namespace foo',
                                'At least two spaces is best between code and comments'
                                '  [whitespace/comments] [2]')
        self.TestSingleLineLint(state, 'printf("foo"); // Outside quotes.',
                                'At least two spaces is best between code and comments'
                                '  [whitespace/comments] [2]')
        ...
        self.TestMultiLineLint(state, 'vector<int> my_elements = {// my_elements is ..\n'
                               ...
                               'At least two spaces is best between code and '
                               'comments  [whitespace/comments] [2]')
        self.TestSingleLineLint(state, 'if (foo) { // not a pure scope; comment is too close!',
                                'At least two spaces is best between code and comments'
                                '  [whitespace/comments] [2]')
    ...
    def testSpaceAfterCommentMarker(self, state: _CppLintState):
        self.TestSingleLineLint(state, '//', '')
        self.TestSingleLineLint(state, '//x', 'Should have a space between // and comment'
                                              '  [whitespace/comments] [4]')
    ...
    def testUsingLiteralsNamespaces(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'using namespace std::literals;', 'Do not use namespace'
                                                                         ' using-directives.  Use using-declarations instead.'
                                                                         '  [build/namespaces_literals] [5]')
        self.TestSingleLineLint(state, 'using namespace std::literals::chrono_literals;', 'Do'
    ...
    def testBlankLinesCheck(self, state: _CppLintState):
    ...
    def testElseClauseNotOnSameLineAsElse(self, state: _CppLintState):
        self.TestSingleLineLint(state, '  else DoSomethingElse();',
                                'Else clause should never be on same line as else '
                                '(use 2 lines)  [whitespace/newline] [4]')
        self.TestSingleLineLint(state, '  else ifDoSomethingElse();',
                                'Else clause should never be on same line as else '
                                '(use 2 lines)  [whitespace/newline] [4]')
        self.TestSingleLineLint(state, '  } else if (blah) {', '')
        self.TestSingleLineLint(state, '  variable_ends_in_else = true;', '')

    def testComma(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'a = f(1,2);',
                                'Missing space after ,  [whitespace/comma] [3]')
        self.TestSingleLineLint(state, 'int tmp=a,a=b,b=tmp;',
        ...
        self.TestSingleLineLint(state, 'operator,(a,b)',
                                'Missing space after ,  [whitespace/comma] [3]')

    def testEqualsOperatorSpacing(self, state):
        self.TestSingleLineLint(state, 'int tmp= a;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int tmp =a;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int tmp=a;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int tmp= 7;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int tmp =7;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int tmp=7;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int* tmp=*p;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'int* tmp= *p;',
                                'Missing spaces around =  [whitespace/operators] [4]')
        self.TestMultiLineLint(state,
            ...
            lookahead_services_=
              ::strings::Split(FLAGS_ls, ",", ::strings::SkipEmpty());'''),
                               'Missing spaces around =  [whitespace/operators] [4]')
        self.TestSingleLineLint(state, 'bool result = a>=42;',
                                'Missing spaces around >=  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'bool result = a<=42;',
                                'Missing spaces around <=  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'bool result = a==42;',
                                'Missing spaces around ==  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'auto result = a!=42;',
                                'Missing spaces around !=  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'int a = b!=c;',
                                'Missing spaces around !=  [whitespace/operators] [3]')
    ...
    def testShiftOperatorSpacing(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'a<<b',
                                'Missing spaces around <<  [whitespace/operators] [3]')
        self.TestSingleLineLint(state, 'a>>b',
                                'Missing spaces around >>  [whitespace/operators] [3]')
    ...
    def testIndent(self, state):
        ...
        self.TestSingleLineLint(state, ' int one_space_indent;',
                                'Weird number of spaces at line-start.  '
                                'Are you using a 2-space indent?  [whitespace/indent] [3]')
        self.TestSingleLineLint(state, '   int three_space_indent;',
                                'Weird number of spaces at line-start.  '
                                'Are you using a 2-space indent?  [whitespace/indent] [3]')
        self.TestSingleLineLint(state, ' char* one_space_indent = "public:";',
                                'Weird number of spaces at line-start.  '
                                'Are you using a 2-space indent?  [whitespace/indent] [3]')
    ...
    def testTab(self, state: _CppLintState):
        self.TestSingleLineLint(state, '\tint16 a;',
    ...
    def testFilter(self, state: _CppLintState):
    ...
    def testDefaultFilter(self, state_with_defaults: _CppLintState):
    ...
    def testUnnamedNamespacesInHeaders(self, state: _CppLintState):
        for extension in ['h', 'hpp', 'hxx', 'h++', 'cuh']:
            self.doTestUnnamedNamespacesInHeaders(state, extension)

    def doTestUnnamedNamespacesInHeaders(self, state: _CppLintState, extension):
    ...
    def testUnnamedNamespacesInNonHeaders(self, state: _CppLintState):
        for extension in ['c', 'cc', 'cpp', 'cxx', 'c++', 'cu']:
            self.TestLanguageRulesCheck(state, 'foo.' + extension, 'namespace {', '')
            self.TestLanguageRulesCheck(state, 'foo.' + extension, 'namespace foo {', '')
    ...
    def testBuildForwardDecl(self, state: _CppLintState):
        # The crosstool compiler we currently use will fail to compile the
        # code in this test, so we might consider removing the lint check.
        self.TestSingleLineLint(state, 'class Foo::Goo;',
    ...
    def doTestBuildHeaderGuardWithRoot(self, header_directory):
        ...
        assert '%sCPPLINT_CPPLINT_TEST_HEADER_H_' % (expected_prefix) == cpplint.GetHeaderGuardCPPVariable(
            cpplint._cpplint_state, file_path)
    ...
    def testBuildHeaderGuardWithRepository(self):
        ...
        try:
            ...
            assert 'TRUNK_CPPLINT_CPPLINT_TEST_HEADER_H_' == cpplint.GetHeaderGuardCPPVariable(cpplint._cpplint_state,
                                                                                               file_path)

            # use the provided repository root for header guards
            cpplint._cpplint_state._repository = os.path.relpath(trunk_dir)
            assert 'CPPLINT_CPPLINT_TEST_HEADER_H_' == cpplint.GetHeaderGuardCPPVariable(cpplint._cpplint_state,
                                                                                         file_path)
    ...
    def testBuildInclude(self, state: _CppLintState):
        # Test that include statements have slashes in them.
        self.TestSingleLineLint(state, '#include "foo.h"',
                                'Include the directory when naming header files'
                                '  [build/include_subdir] [4]')
        self.TestSingleLineLint(state, '#include "bar.hh"',
                                'Include the directory when naming header files'
                                '  [build/include_subdir] [4]')
    ...
    def testBuildStorageClass(self, state: _CppLintState):
        ...
        self.TestSingleLineLint(state, 'const int static foo = 5;',
                                build_storage_class_error_message)

        self.TestSingleLineLint(state, 'char static foo;',
                                build_storage_class_error_message)

        self.TestSingleLineLint(state, 'double const static foo = 2.0;',
                                build_storage_class_error_message)

        self.TestSingleLineLint(state, 'uint64 typedef unsigned_long_long;',
                                build_storage_class_error_message)

        self.TestSingleLineLint(state, 'int register foo = 0;',
                                build_storage_class_error_message)
        ...
        for unused_i in range(10):
            ...
            self.TestSingleLineLintLogCodeOnError(state,
                                                  ' '.join(decl_specs) + ';',
                                                  build_storage_class_error_message)
    ...
    def testInvalidIncrement(self, state: _CppLintState):
        self.TestSingleLineLint(state, '*count++;',
                                'Changing pointer instead of value (or unused value of '
                                'operator*).  [runtime/invalid_increment] [5]')

    def testSnprintfSize(self, state: _CppLintState):
        self.TestSingleLineLint(state, 'vsnprintf(NULL, 0, format)', '')
        self.TestSingleLineLint(state, 'snprintf(fisk, 1, format)',
