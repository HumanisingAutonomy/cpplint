...
class TestCpplint(CpplintTestBase):
    ...
    def testForwardDeclarationNameSpaceIndentation(self, state: LintState):
        lines = [
            "namespace Test {",
        ...
        results = self.GetNamespaceResults(state, lines)
        assert results == "Do not indent within a namespace  [runtime/indentation_namespace] [4]"

    def testNameSpaceIndentationForClass(self, state: LintState):
        lines = [
            "namespace Test {",
            "void foo() { }",
            "  class Test {",
    ...
    def testNameSpaceIndentationNoError(self, state):
        lines = ["namespace Test {", "void foo() { }", "}  // namespace Test"]

        results = self.GetNamespaceResults(state, lines)
        assert results == ""

    def testWhitespaceBeforeNamespace(self, state):
        lines = ["  namespace Test {", "  void foo() { }", "  }  // namespace Test"]

        results = self.GetNamespaceResults(state, lines)
        assert results == ""

    def testFalsePositivesNoError(self, state):
        lines = [
            "namespace Test {",
            "struct OuterClass {",
    ...
    def testGetLineWidth(self):
        ...
        assert 16 == GetLineWidth("都|道|府|県|支庁")
        assert 5 + 13 + 9 == GetLineWidth("d𝐱/dt" + "f : t ⨯ 𝐱 → ℝ" + "t ⨯ 𝐱 → ℝ")
    ...
    def testGetTextInside(self):
        ...
        assert "x, y{}" == _GetTextInside("f[x, y{}]", r"f\[")
        assert None is _GetTextInside("f[a, b(}]", r"f\[")
        assert None is _GetTextInside("f[x, y]", r"f\(")
    ...
    def testSpacesAtEndOfLine(self, state):
    ...
    def testLineLengthCheck(self, state: LintState, code: str, expected_message: str):
    ...
    # to bad casts.
    def testMockCallback(self, state):
        self.TestSingleLineLint(state, "MockCallback<bool(int)>", "")
        self.TestSingleLineLint(state, "MockCallback<int(float, char)>", "")

    # Test false errors that happened with some include file names
    def testIncludeFilenameFalseError(self, state):
        self.TestSingleLineLint(state, '#include "foo/long-foo.h"', "")
        self.TestSingleLineLint(state, '#include "foo/sprintf.h"', "")
    ...
    def testTypedefForPointerToFunction(self, state):
        self.TestSingleLineLint(state, "typedef void (*Func)(int x);", "")
        self.TestSingleLineLint(state, "typedef void (*Func)(int *x);", "")
        self.TestSingleLineLint(state, "typedef void Func(int x);", "")
        self.TestSingleLineLint(state, "typedef void Func(int *x);", "")

    def testIncludeWhatYouUseNoImplementationFiles(self, state: LintState):
        code = "std::vector<int> foo;"
        for extension in ["h", "hpp", "hxx", "h++", "cuh"]:
            assert (
                "Add #include <vector> for vector<>  [build/include_what_you_use] [4]"
                == self.PerformIncludeWhatYouUse(state, code, "foo." + extension)
            )
        for extension in ["c", "cc", "cpp", "cxx", "c++", "cu"]:
            assert "" == self.PerformIncludeWhatYouUse(state, code, "foo." + extension)

    def testIncludeWhatYouUse(self, state: LintState):
        ...
        self.TestIncludeWhatYouUse(
            state, "void a(const my::string &foobar);", ""
    ...
    def testFilesBelongToSameModule(self, state):
        f = partial(cpplint.FilesBelongToSameModule, state)
    ...
    def testMultiLineComments(self, state: LintState, code, expected_message):
        self.TestLint(state, code, expected_message)
    ...
    def testExplicitSingleArgumentConstructors(self, state):
        ...
        assert 0 == error_collector.ResultList().count(
            "Constructors that require multiple arguments should not be marked explicit.  [runtime/explicit] [0]"
        )
        error_collector = ErrorCollector()
        cpplint.ProcessFileData(
            state,
    ...
    def testSlashStarCommentOnSingleLine(self, state):
        self.TestMultiLineLint(state, """/* static */ Foo(int f);""", "")
        self.TestMultiLineLint(state, """/*/ static */  Foo(int f);""", "")
    ...
    # }
    def testSuspiciousUsageOfIf(self, state):
        self.TestSingleLineLint(state, "  if (a == b) {", "")
        self.TestSingleLineLint(
            state,
            "  } if (a == b) {",
    ...
    def testSuspiciousUsageOfMemset(self, state):
        # Normal use is okay.
        self.TestSingleLineLint(state, "  memset(buf, 0, sizeof(buf))", "")
        ...
        self.TestSingleLineLint(state, "  memset(buf, 'y', 0)", "")
        self.TestSingleLineLint(state, "  memset(buf, 4, 0)", "")
        self.TestSingleLineLint(state, "  memset(buf, -1, 0)", "")
        self.TestSingleLineLint(state, "  memset(buf, 0xF1, 0)", "")
        self.TestSingleLineLint(state, "  memset(buf, 0xcd, 0)", "")

    def testRedundantVirtual(self, state):
        self.TestSingleLineLint(state, "virtual void F()", "")
        self.TestSingleLineLint(state, "virtual void F();", "")
        self.TestSingleLineLint(state, "virtual void F() {}", "")

        message_template = (
            '"%s" is redundant since function is already ' 'declared as "%s"  [readability/inheritance] [4]'
        )
        for virt_specifier in ["override", "final"]:
            error_message = message_template % ("virtual", virt_specifier)
            self.TestSingleLineLint(state, "virtual int F() %s" % virt_specifier, error_message)
            self.TestSingleLineLint(state, "virtual int F() %s;" % virt_specifier, error_message)
            self.TestSingleLineLint(state, "virtual int F() %s {" % virt_specifier, error_message)
        ...
        self.TestSingleLineLint(state, "int F() override final", error_message)
        self.TestSingleLineLint(state, "int F() override final;", error_message)
        self.TestSingleLineLint(state, "int F() override final {}", error_message)
        self.TestSingleLineLint(state, "int F() final override", error_message)
        self.TestSingleLineLint(state, "int F() final override;", error_message)
        self.TestSingleLineLint(state, "int F() final override {}", error_message)
        ...
        self.TestSingleLineLint(state, "void Finalize(AnnotationProto *final) override;", "")

    def testCheckDeprecated(self, state: LintState):
        self.TestLanguageRulesCheck(state, "foo_test.cc", "#include <iostream>", "")
        self.TestLanguageRulesCheck(state, "foo_unittest.cc", "#include <iostream>", "")

    def testCheckPosixThreading(self, state):
        self.TestSingleLineLint(state, "var = sctime_r()", "")
        self.TestSingleLineLint(state, "var = strtok_r()", "")
        self.TestSingleLineLint(state, "var = strtok_r(foo, ba, r)", "")
        self.TestSingleLineLint(state, "var = brand()", "")
        self.TestSingleLineLint(state, "_rand()", "")
        self.TestSingleLineLint(state, ".rand()", "")
        self.TestSingleLineLint(state, "->rand()", "")
        self.TestSingleLineLint(state, "ACMRandom rand(seed)", "")
        self.TestSingleLineLint(state, "ISAACRandom rand()", "")
        self.TestSingleLineLint(
            state,
            "var = rand()",
    ...
    def testVlogMisuse(self, state):
        self.TestSingleLineLint(state, "VLOG(1)", "")
        self.TestSingleLineLint(state, "VLOG(99)", "")
        self.TestSingleLineLint(state, "LOG(ERROR)", "")
        self.TestSingleLineLint(state, "LOG(INFO)", "")
        self.TestSingleLineLint(state, "LOG(WARNING)", "")
        self.TestSingleLineLint(state, "LOG(FATAL)", "")
        self.TestSingleLineLint(state, "LOG(DFATAL)", "")
        self.TestSingleLineLint(state, "VLOG(SOMETHINGWEIRD)", "")
        self.TestSingleLineLint(state, "MYOWNVLOG(ERROR)", "")
        errmsg = (
            "VLOG() should be used with numeric verbosity level.  "
            "Use LOG() if you want symbolic severity levels."
            "  [runtime/vlog] [5]"
        )
        self.TestSingleLineLint(state, "VLOG(ERROR)", errmsg)
        self.TestSingleLineLint(state, "VLOG(INFO)", errmsg)
        self.TestSingleLineLint(state, "VLOG(WARNING)", errmsg)
        self.TestSingleLineLint(state, "VLOG(FATAL)", errmsg)
        self.TestSingleLineLint(state, "VLOG(DFATAL)", errmsg)
        self.TestSingleLineLint(state, "  VLOG(ERROR)", errmsg)
        self.TestSingleLineLint(state, "  VLOG(INFO)", errmsg)
        self.TestSingleLineLint(state, "  VLOG(WARNING)", errmsg)
        self.TestSingleLineLint(state, "  VLOG(FATAL)", errmsg)
    ...
    def testFormatStrings(self, state):
        self.TestSingleLineLint(state, 'printf("foo")', "")
        self.TestSingleLineLint(state, 'printf("foo: %s", foo)', "")
        self.TestSingleLineLint(state, "DocidForPrintf(docid)", "")  # Should not trigger.
        self.TestSingleLineLint(state, "printf(format, value)", "")  # Should not trigger.
        self.TestSingleLineLint(state, "printf(__VA_ARGS__)", "")  # Should not trigger.
        self.TestSingleLineLint(state, "printf(format.c_str(), value)", "")  # Should not trigger.
        self.TestSingleLineLint(state, "printf(format(index).c_str(), value)", "")
        self.TestSingleLineLint(
            state,
            "printf(foo)",
        ...
        self.TestSingleLineLint(
            state,
            "printf(foo.c_str())",
            "Potential format string bug. " 'Do printf("%s", foo.c_str()) instead.' "  [runtime/printf] [4]",
        )
        self.TestSingleLineLint(
            state,
            "printf(foo->c_str())",
        ...
        self.TestSingleLineLint(
            state,
            "StringPrintf(foo)",
            'Potential format string bug. Do StringPrintf("%s", foo) instead.' "" "  [runtime/printf] [4]",
        )

    # Test disallowed use of operator& and other operators.
    def testIllegalOperatorOverloading(self, state):
        errmsg = "Unary operator& is dangerous.  Do not use it." "  [runtime/operator] [4]"
        self.TestSingleLineLint(state, "void operator=(const Myclass&)", "")
        self.TestSingleLineLint(state, "void operator&(int a, int b)", "")  # binary operator& ok
        self.TestSingleLineLint(state, "void operator&() { }", errmsg)
        self.TestSingleLineLint(
            state,
            "void operator & (  ) { }",
    ...
    def testConstStringReferenceMembers(self, state):
        errmsg = (
        ...
        self.TestSingleLineLint(state, "void f(const string&)", "")
        self.TestSingleLineLint(state, "const string& f(const string& a, const string& b)", "")
        self.TestSingleLineLint(state, "typedef const string& A;", "")

        for decl in members_declarations:
            self.TestSingleLineLint(state, decl + " = b;", "")
            self.TestSingleLineLint(state, decl + "      =", "")

        # The Bad.

        for decl in members_declarations:
            self.TestSingleLineLint(state, decl + ";", errmsg)

    # Variable-length arrays are not permitted.
    def testVariableLengthArrayDetection(self, state):
        errmsg = (
        ...
        self.TestSingleLineLint(state, "int a[any_old_variable];", errmsg)
        self.TestSingleLineLint(state, "int doublesize[some_var * 2];", errmsg)
        self.TestSingleLineLint(state, "int a[afunction()];", errmsg)
        self.TestSingleLineLint(state, "int a[function(kMaxFooBars)];", errmsg)
        self.TestSingleLineLint(state, "bool a_list[items_->size()];", errmsg)
        self.TestSingleLineLint(state, "namespace::Type buffer[len+1];", errmsg)

        self.TestSingleLineLint(state, "int a[64];", "")
        self.TestSingleLineLint(state, "int a[0xFF];", "")
        self.TestSingleLineLint(state, "int first[256], second[256];", "")
        self.TestSingleLineLint(state, "int array_name[kCompileTimeConstant];", "")
        self.TestSingleLineLint(state, "char buf[somenamespace::kBufSize];", "")
        self.TestSingleLineLint(state, "int array_name[ALL_CAPS];", "")
        self.TestSingleLineLint(state, "AClass array1[foo::bar::ALL_CAPS];", "")
        self.TestSingleLineLint(state, "int a[kMaxStrLen + 1];", "")
        self.TestSingleLineLint(state, "int a[sizeof(foo)];", "")
        self.TestSingleLineLint(state, "int a[sizeof(*foo)];", "")
        self.TestSingleLineLint(state, "int a[sizeof foo];", "")
        self.TestSingleLineLint(state, "int a[sizeof(struct Foo)];", "")
        self.TestSingleLineLint(state, "int a[128 - sizeof(const bar)];", "")
        self.TestSingleLineLint(state, "int a[(sizeof(foo) * 4)];", "")
        self.TestSingleLineLint(state, "int a[(arraysize(fixed_size_array)/2) << 1];", "")
        self.TestSingleLineLint(state, "delete a[some_var];", "")
        self.TestSingleLineLint(state, "return a[some_var];", "")
    ...
    def testDisallowMacrosAtEnd(self, state):
        for macro_name in (
            ...
            ) + "  [readability/constructors] [3]" == error_collector.Results()
    ...
    def testBraces(self, state):
        ...
        self.TestSingleLineLint(state, "int a[3] = { 1, 2, 3 };", "")
        self.TestSingleLineLint(
            state,
            """const int foo[] =
               {1, 2, 3 };""",
    ...
    def testCheckCheck(self, state):
        ...
        self.TestSingleLineLint(state, "using some::namespace::operator<<;", "")
        self.TestSingleLineLint(state, "using some::namespace::operator>>;", "")
    ...
    def testCheckCheckFalsePositives(self, state):
        self.TestSingleLineLint(state, "CHECK(some_iterator == obj.end());", "")
        self.TestSingleLineLint(state, "EXPECT_TRUE(some_iterator == obj.end());", "")
        self.TestSingleLineLint(state, "EXPECT_FALSE(some_iterator == obj.end());", "")
        self.TestSingleLineLint(state, "CHECK(some_pointer != NULL);", "")
        self.TestSingleLineLint(state, "EXPECT_TRUE(some_pointer != NULL);", "")
        self.TestSingleLineLint(state, "EXPECT_FALSE(some_pointer != NULL);", "")

        self.TestSingleLineLint(state, "CHECK(CreateTestFile(dir, (1 << 20)));", "")
        self.TestSingleLineLint(state, "CHECK(CreateTestFile(dir, (1 >> 20)));", "")

        self.TestSingleLineLint(state, "CHECK(x ^ (y < 42));", "")
        self.TestSingleLineLint(state, "CHECK((x > 42) ^ (x < 54));", "")
        self.TestSingleLineLint(state, "CHECK(a && b < 42);", "")
        self.TestSingleLineLint(state, "CHECK(42 < a && a < b);", "")
        self.TestSingleLineLint(state, "SOFT_CHECK(x > 42);", "")
        ...
        self.TestSingleLineLint(state, 'CHECK(x < 42) << "Custom error message";', "")

    # Alternative token to punctuation operator replacements
    def testCheckAltTokens(self, state):
        self.TestSingleLineLint(
            state,
            "true or true",
        ...
        self.TestSingleLineLint(
            state,
            "1 bitor 1",
            "Use operator | instead of bitor" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
            "1 xor 1",
        ...
        self.TestSingleLineLint(
            state,
            "1 bitand 1",
            "Use operator & instead of bitand" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
            "x = compl 1",
            "Use operator ~ instead of compl" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
            "x and_eq y",
            "Use operator &= instead of and_eq" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
            "x or_eq y",
            "Use operator |= instead of or_eq" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
            "x xor_eq y",
            "Use operator ^= instead of xor_eq" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
            "x not_eq y",
            "Use operator != instead of not_eq" "  [readability/alt_tokens] [2]",
        )
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(state, '#include "base/false-and-false.h"', "")
        self.TestSingleLineLint(state, "#error false or false", "")
        self.TestSingleLineLint(state, "false nor false", "")
        self.TestSingleLineLint(state, "false nand false", "")
    ...
    def testNonConstReference(self, state):
        ...
        operand_error_message = (
            "Is this a non-const reference? " "If so, make const or use a pointer: %s" "  [runtime/references] [2]"
        )
        ...
        self.TestSingleLineLint(state, "int len(Foo& s);", operand_error_message % "Foo& s")
        # Allow use of non-const references in a few specific cases
        self.TestSingleLineLint(state, "stream& operator>>(stream& s, Foo& f);", "")
        self.TestSingleLineLint(state, "stream& operator<<(stream& s, Foo& f);", "")
        self.TestSingleLineLint(state, "void swap(Bar& a, Bar& b);", "")
        self.TestSingleLineLint(state, "ostream& LogFunc(ostream& s);", "")
        self.TestSingleLineLint(state, "ostringstream& LogFunc(ostringstream& s);", "")
        self.TestSingleLineLint(state, "istream& LogFunc(istream& s);", "")
        self.TestSingleLineLint(state, "istringstream& LogFunc(istringstream& s);", "")
        # Returning a non-const reference from a function is OK.
        self.TestSingleLineLint(state, "int& g();", "")
        # Passing a const reference to a struct (using the struct keyword) is OK.
        self.TestSingleLineLint(state, "void foo(const struct tm& tm);", "")
        # Passing a const reference to a typename is OK.
        self.TestSingleLineLint(state, "void foo(const typename tm& tm);", "")
        # Const reference to a pointer type is OK.
        self.TestSingleLineLint(state, "void foo(const Bar* const& p) {", "")
        self.TestSingleLineLint(state, "void foo(Bar const* const& p) {", "")
        self.TestSingleLineLint(state, "void foo(Bar* const& p) {", "")
        # Const reference to a templated type is OK.
        self.TestSingleLineLint(state, "void foo(const std::vector<std::string>& v);", "")
        # Non-const reference to a pointer type is not OK.
        self.TestSingleLineLint(state, "void foo(Bar*& p);", operand_error_message % "Bar*& p")
        self.TestSingleLineLint(state, "void foo(const Bar*& p);", operand_error_message % "const Bar*& p")
        self.TestSingleLineLint(state, "void foo(Bar const*& p);", operand_error_message % "Bar const*& p")
        self.TestSingleLineLint(state, "void foo(struct Bar*& p);", operand_error_message % "struct Bar*& p")
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(state, "return &something;", "")
        self.TestSingleLineLint(state, "if (condition) {return &something; }", "")
        self.TestSingleLineLint(state, "if (condition) return &something;", "")
        self.TestSingleLineLint(state, "if (condition) address = &something;", "")
        self.TestSingleLineLint(state, "if (condition) result = lhs&rhs;", "")
        self.TestSingleLineLint(state, "if (condition) result = lhs & rhs;", "")
        self.TestSingleLineLint(state, "a = (b+c) * sizeof &f;", "")
        self.TestSingleLineLint(state, "a = MySize(b) * sizeof &f;", "")
        # We don't get confused by C++11 range-based for loops.
        self.TestSingleLineLint(state, "for (const string& s : c)", "")
        self.TestSingleLineLint(state, "for (auto& r : c)", "")
        self.TestSingleLineLint(state, "for (typename Type& a : b)", "")
        # We don't get confused by some other uses of '&'.
        self.TestSingleLineLint(state, "T& operator=(const T& t);", "")
        self.TestSingleLineLint(state, "int g() { return (a & b); }", "")
        self.TestSingleLineLint(state, "T& r = (T&)*(vp());", "")
        self.TestSingleLineLint(state, "T& r = v", "")
        self.TestSingleLineLint(state, 'static_assert((kBits & kMask) == 0, "text");', "")
        self.TestSingleLineLint(state, "COMPILE_ASSERT((kBits & kMask) == 0, text);", "")
        ...
        self.TestSingleLineLint(state, "void Func(X& x);", operand_error_message % "X& x")
        self.TestSingleLineLint(state, "void Func(X& x) {}", operand_error_message % "X& x")
        self.TestSingleLineLint(state, "void Func(X& x) override;", "")
        self.TestSingleLineLint(state, "void Func(X& x) override {", "")
        self.TestSingleLineLint(state, "void Func(X& x) const override;", "")
        self.TestSingleLineLint(state, "void Func(X& x) const override {", "")

        # Don't warn on out-of-line method definitions.
        self.TestSingleLineLint(state, "void NS::Func(X& x) {", "")
        error_collector = ErrorCollector()
        cpplint.ProcessFileData(
            state,
            "foo.cc",
    ...
    def testMismatchingSpacesInParens(self, state):
        self.TestSingleLineLint(
            state,
            "if (foo ) {",
        ...
        self.TestSingleLineLint(state, "for (; foo; bar) {", "")
        self.TestSingleLineLint(state, "for ( ; foo; bar) {", "")
        self.TestSingleLineLint(state, "for ( ; foo; bar ) {", "")
        self.TestSingleLineLint(state, "for (foo; bar; ) {", "")
        self.TestSingleLineLint(
    ...
    def testSpacingForFncall(self, state):
        self.TestSingleLineLint(state, "if (foo) {", "")
        self.TestSingleLineLint(state, "for (foo; bar; baz) {", "")
        self.TestSingleLineLint(state, "for (;;) {", "")
        # Space should be allowed in placement new operators.
        self.TestSingleLineLint(state, "Something* p = new (place) Something();", "")
        # Test that there is no warning when increment statement is empty.
        self.TestSingleLineLint(state, "for (foo; baz;) {", "")
        self.TestSingleLineLint(
            state,
            "for (foo;bar;baz) {",
        ...
        self.TestSingleLineLint(state, "if (condition) {return &something; }", "")
        # seen in some macros
        self.TestSingleLineLint(state, "DoSth();\\", "")
        ...
        self.TestSingleLineLint(state, "while (foo) {", "")
        self.TestSingleLineLint(state, "switch (foo) {", "")
        self.TestSingleLineLint(
            state,
            "foo( bar)",
            "Extra space after ( in function call" "  [whitespace/parens] [4]",
        )
        self.TestSingleLineLint(state, "foo(  // comment", "")
        self.TestSingleLineLint(
            state,
            "foo( // comment",
            "At least two spaces is best between code" " and comments  [whitespace/comments] [2]",
        )
        self.TestSingleLineLint(state, "foobar( \\", "")
        self.TestSingleLineLint(state, "foobar(     \\", "")
        self.TestSingleLineLint(state, "( a + b)", "Extra space after (" "  [whitespace/parens] [2]")
        self.TestSingleLineLint(state, "((a+b))", "")
        self.TestSingleLineLint(
            state,
            "foo (foo)",
            "Extra space before ( in function call" "  [whitespace/parens] [4]",
        ...
        self.TestSingleLineLint(state, 'asm volatile ("")', "")
        self.TestSingleLineLint(state, '__asm__ __volatile__ ("")', "")
        self.TestSingleLineLint(state, "} catch (const Foo& ex) {", "")
        self.TestSingleLineLint(state, "case (42):", "")
        self.TestSingleLineLint(state, "typedef foo (*foo)(foo)", "")
        self.TestSingleLineLint(state, "typedef foo (*foo12bar_)(foo)", "")
        self.TestSingleLineLint(state, "typedef foo (Foo::*bar)(foo)", "")
        self.TestSingleLineLint(state, "using foo = type (Foo::*bar)(foo)", "")
        self.TestSingleLineLint(state, "using foo = type (Foo::*bar)(", "")
        self.TestSingleLineLint(state, "using foo = type (Foo::*)(", "")
        self.TestSingleLineLint(state, "foo (Foo::*bar)(", "")
        self.TestSingleLineLint(state, "foo (x::y::*z)(", "")
        self.TestSingleLineLint(
            state,
            "foo (Foo::bar)(",
            "Extra space before ( in function call" "  [whitespace/parens] [4]",
        )
        self.TestSingleLineLint(state, "foo (*bar)(", "")
        self.TestSingleLineLint(state, "typedef foo (Foo::*bar)(", "")
        self.TestSingleLineLint(state, "(foo)(bar)", "")
        self.TestSingleLineLint(state, "Foo (*foo)(bar)", "")
        self.TestSingleLineLint(state, "Foo (*foo)(Bar bar,", "")
        self.TestSingleLineLint(state, "char (*p)[sizeof(foo)] = &foo", "")
        self.TestSingleLineLint(state, "char (&ref)[sizeof(foo)] = &foo", "")
        self.TestSingleLineLint(state, "const char32 (*table[])[6];", "")
        ...
        self.TestSingleLineLint(state, "sizeof(foo)", "")
        self.TestSingleLineLint(state, "sizeof foo", "")
        self.TestSingleLineLint(state, "sizeof (foo)", "")

    def testSpacingBeforeBraces(self, state: LintState):
        self.TestSingleLineLint(state, "if (foo){", "Missing space before {" "  [whitespace/braces] [5]")
        self.TestSingleLineLint(state, "for{", "Missing space before {" "  [whitespace/braces] [5]")
        self.TestSingleLineLint(state, "for {", "")
        self.TestSingleLineLint(state, "EXPECT_DEBUG_DEATH({", "")
        self.TestSingleLineLint(state, "std::is_convertible<A, B>{}", "")
        self.TestSingleLineLint(state, "blah{32}", "Missing space before {" "  [whitespace/braces] [5]")
        self.TestSingleLineLint(state, "int8_t{3}", "")
        self.TestSingleLineLint(state, "int16_t{3}", "")
        self.TestSingleLineLint(state, "int32_t{3}", "")
        self.TestSingleLineLint(state, "uint64_t{12345}", "")
        self.TestSingleLineLint(
            state,
    ...
    def testSemiColonAfterBraces(self, state):
        ...
        self.TestSingleLineLint(state, "class X {};", "")
            ...
            for align in ["", " alignas(16)"]:
                for typename in ["", " X"]:
                    for identifier in ["", " x"]:
                        self.TestSingleLineLint(
                            state,
                            keyword + align + typename + " {}" + identifier + ";",
                            "",
                        )

        self.TestSingleLineLint(state, "class X : public Y {};", "")
        self.TestSingleLineLint(state, "class X : public MACRO() {};", "")
        self.TestSingleLineLint(state, "class X : public decltype(expr) {};", "")
        self.TestSingleLineLint(state, "DEFINE_FACADE(PCQueue::Watcher, PCQueue) {};", "")
        self.TestSingleLineLint(state, "VCLASS(XfaTest, XfaContextTest) {};", "")
        self.TestSingleLineLint(state, "class STUBBY_CLASS(H, E) {};", "")
        self.TestSingleLineLint(state, "class STUBBY2_CLASS(H, E) {};", "")
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(state, "file_tocs_[i] = (FileToc) {a, b, c};", "")
        self.TestMultiLineLint(state, "class X : public Y,\npublic Z {};", "")

    def testSpacingBeforeBrackets(self, state: LintState):
        self.TestSingleLineLint(
            state,
            "int numbers [] = { 1, 2, 3 };",
        ...
        self.TestSingleLineLint(state, "auto [abc, def] = func();", "")
        self.TestSingleLineLint(state, "#define NODISCARD [[nodiscard]]", "")
        self.TestSingleLineLint(state, "void foo(int param [[maybe_unused]]);", "")

    def testLambda(self, state):
        self.TestSingleLineLint(state, "auto x = []() {};", "")
        self.TestSingleLineLint(state, "return []() {};", "")
        ...
        self.TestSingleLineLint(state, "table_to_children[&*table].push_back(dependent);", "")

    def testBraceInitializerList(self, state):
        self.TestSingleLineLint(state, "MyStruct p = {1, 2};", "")
        self.TestSingleLineLint(state, "MyStruct p{1, 2};", "")
        self.TestSingleLineLint(state, "vector<int> p = {1, 2};", "")
        self.TestSingleLineLint(state, "vector<int> p{1, 2};", "")
        self.TestSingleLineLint(state, "x = vector<int>{1, 2};", "")
        self.TestSingleLineLint(state, "x = (struct in_addr){ 0 };", "")
        self.TestSingleLineLint(state, "Func(vector<int>{1, 2})", "")
        self.TestSingleLineLint(state, "Func((struct in_addr){ 0 })", "")
        self.TestSingleLineLint(state, "Func(vector<int>{1, 2}, 3)", "")
        self.TestSingleLineLint(state, "Func((struct in_addr){ 0 }, 3)", "")
        self.TestSingleLineLint(state, "LOG(INFO) << char{7};", "")
        self.TestSingleLineLint(state, 'LOG(INFO) << char{7} << "!";', "")
        self.TestSingleLineLint(state, "int p[2] = {1, 2};", "")
        self.TestSingleLineLint(state, "return {1, 2};", "")
        self.TestSingleLineLint(state, "std::unique_ptr<Foo> foo{new Foo{}};", "")
        self.TestSingleLineLint(state, "auto foo = std::unique_ptr<Foo>{new Foo{}};", "")
        self.TestSingleLineLint(state, 'static_assert(Max7String{}.IsValid(), "");', "")
        self.TestSingleLineLint(state, "map_of_pairs[{1, 2}] = 3;", "")
        self.TestSingleLineLint(state, "ItemView{has_offer() ? new Offer{offer()} : nullptr", "")
        self.TestSingleLineLint(state, "template <class T, EnableIf<::std::is_const<T>{}> = 0>", "")
    ...
    def testSpacingAroundElse(self, state: LintState):
        self.TestSingleLineLint(state, "}else {", "Missing space before else" "  [whitespace/braces] [5]")
        self.TestSingleLineLint(state, "} else{", "Missing space before {" "  [whitespace/braces] [5]")
        self.TestSingleLineLint(state, "} else {", "")
        self.TestSingleLineLint(state, "} else if (foo) {", "")

    def testSpacingWithInitializerLists(self, state: LintState):
        self.TestSingleLineLint(state, "int v[1][3] = {{1, 2, 3}};", "")
        self.TestSingleLineLint(state, "int v[1][1] = {{0}};", "")

    def testSpacingForBinaryOps(self, state: LintState):
        self.TestSingleLineLint(
            state,
            "if (foo||bar) {",
            "Missing spaces around ||" "  [whitespace/operators] [3]",
        ...
        self.TestSingleLineLint(state, "template<typename T = double>", "")
        self.TestSingleLineLint(state, "std::unique_ptr<No<Spaces>>", "")
        self.TestSingleLineLint(state, "typedef hash_map<Foo, Bar>", "")
        self.TestSingleLineLint(state, "10<<20", "")
        self.TestSingleLineLint(state, "10<<a", "Missing spaces around <<  [whitespace/operators] [3]")
        self.TestSingleLineLint(state, "a<<20", "Missing spaces around <<  [whitespace/operators] [3]")
        self.TestSingleLineLint(state, "a<<b", "Missing spaces around <<  [whitespace/operators] [3]")
        self.TestSingleLineLint(state, "10LL<<20", "")
        self.TestSingleLineLint(state, "10ULL<<20", "")
        self.TestSingleLineLint(state, "a>>b", "Missing spaces around >>  [whitespace/operators] [3]")
        self.TestSingleLineLint(state, "10>>b", "Missing spaces around >>  [whitespace/operators] [3]")
        self.TestSingleLineLint(
            state,
            "LOG(ERROR)<<*foo",
            "Missing spaces around <<  [whitespace/operators] [3]",
        ...
        self.TestSingleLineLint(state, "StringCoder<vector<string>>::ToString()", "")
        self.TestSingleLineLint(state, "map<pair<int16, int16>, map<int16, int16>>::iterator", "")
        self.TestSingleLineLint(state, "func<int16, pair<int16, pair<int16, int16>>>()", "")
        self.TestSingleLineLint(state, "MACRO1(list<list<int16>>)", "")
        self.TestSingleLineLint(state, "MACRO2(list<list<int16>>, 42)", "")
        self.TestSingleLineLint(state, "void DoFoo(const set<vector<string>>& arg1);", "")
        self.TestSingleLineLint(state, "void SetFoo(set<vector<string>>* arg1);", "")
        self.TestSingleLineLint(state, "foo = new set<vector<string>>;", "")
        self.TestSingleLineLint(state, "reinterpret_cast<set<vector<string>>*>(a);", "")
        self.TestSingleLineLint(state, "MACRO(<<)", "")
        self.TestSingleLineLint(state, "MACRO(<<, arg)", "")
        self.TestSingleLineLint(state, "MACRO(<<=)", "")
        self.TestSingleLineLint(state, "MACRO(<<=, arg)", "")

        self.TestSingleLineLint(state, "using Vector3<T>::operator==;", "")
        self.TestSingleLineLint(state, "using Vector3<T>::operator!=;", "")

    def testSpacingBeforeLastSemicolon(self, state: LintState):
        self.TestSingleLineLint(
            state,
            "call_function() ;",
        ...
        self.TestSingleLineLint(state, "for (int i = 0; ;", "")
    ...
    def testEmptyBlockBody(self, state):
        ...
        self.TestSingleLineLint(state, "while (true)", "")
        self.TestSingleLineLint(state, "while (true) continue;", "")
        self.TestSingleLineLint(
            state,
            "for (;;);",
            "Empty loop bodies should use {} or continue" "  [whitespace/empty_loop_body] [5]",
        ...
        self.TestSingleLineLint(state, "for (;;)", "")
        self.TestSingleLineLint(state, "for (;;) continue;", "")
        self.TestSingleLineLint(state, "for (;;) func();", "")
        self.TestSingleLineLint(
            state,
            "if (test) {}",
            "If statement had no body and no else clause" "  [whitespace/empty_if_body] [4]",
        )
        self.TestSingleLineLint(state, "if (test) func();", "")
        self.TestSingleLineLint(state, "if (test) {} else {}", "")
        self.TestMultiLineLint(
            state,
            """while (true &&
                                     false);""",
        ...
        self.TestSingleLineLint(state, "if (test) { hello; }", "")
        self.TestSingleLineLint(state, "if (test({})) { hello; }", "")
        self.TestMultiLineLint(
            state,
            """if (test) {
    ...
    def testSpacingForRangeBasedFor(self, state: LintState):
        # Basic correctly formatted case:
        self.TestSingleLineLint(state, "for (int i : numbers) {", "")
        ...
        self.TestSingleLineLint(state, "for (std::size_t i : sizes) {}", "")
    ...
    def testStaticOrGlobalSTLStrings(self, state):
        ...
        error_msg = (
            "For a static/global string constant, use a C style " 'string instead: "%s[]".  [runtime/string] [4]'
        )
        # The error message for a non-const global/static string variable.
        nonconst_error_msg = "Static/global string variables are not permitted." "  [runtime/string] [4]"

        self.TestSingleLineLint(state, "string foo;", nonconst_error_msg)
        self.TestSingleLineLint(state, 'string kFoo = "hello";  // English', nonconst_error_msg)
        self.TestSingleLineLint(state, "static string foo;", nonconst_error_msg)
        self.TestSingleLineLint(state, "static const string foo;", error_msg % "static const char foo")
        self.TestSingleLineLint(state, "static const std::string foo;", error_msg % "static const char foo")
        self.TestSingleLineLint(state, "string Foo::bar;", nonconst_error_msg)

        self.TestSingleLineLint(state, "std::string foo;", nonconst_error_msg)
        self.TestSingleLineLint(state, 'std::string kFoo = "hello";  // English', nonconst_error_msg)
        self.TestSingleLineLint(state, "static std::string foo;", nonconst_error_msg)
        self.TestSingleLineLint(state, "static const std::string foo;", error_msg % "static const char foo")
        self.TestSingleLineLint(state, "std::string Foo::bar;", nonconst_error_msg)
        ...
        self.TestSingleLineLint(state, "string* pointer", "")
        self.TestSingleLineLint(state, "string *pointer", "")
        self.TestSingleLineLint(state, "string* pointer = Func();", "")
        self.TestSingleLineLint(state, "string *pointer = Func();", "")
        self.TestSingleLineLint(state, "const string* pointer", "")
        self.TestSingleLineLint(state, "const string *pointer", "")
        self.TestSingleLineLint(state, "const string* pointer = Func();", "")
        self.TestSingleLineLint(state, "const string *pointer = Func();", "")
        self.TestSingleLineLint(state, "string const* pointer", "")
        self.TestSingleLineLint(state, "string const *pointer", "")
        self.TestSingleLineLint(state, "string const* pointer = Func();", "")
        self.TestSingleLineLint(state, "string const *pointer = Func();", "")
        self.TestSingleLineLint(state, "string* const pointer", "")
        self.TestSingleLineLint(state, "string *const pointer", "")
        self.TestSingleLineLint(state, "string* const pointer = Func();", "")
        self.TestSingleLineLint(state, "string *const pointer = Func();", "")
        self.TestSingleLineLint(state, "string Foo::bar() {}", "")
        self.TestSingleLineLint(state, "string Foo::operator*() {}", "")
        # Rare case.
        self.TestSingleLineLint(state, 'string foo("foobar");', nonconst_error_msg)
        # Should not catch local or member variables.
        self.TestSingleLineLint(state, "  string foo", "")
        # Should not catch functions.
        self.TestSingleLineLint(state, 'string EmptyString() { return ""; }', "")
        self.TestSingleLineLint(state, 'string EmptyString () { return ""; }', "")
        self.TestSingleLineLint(state, "string const& FileInfo::Pathname() const;", "")
        self.TestSingleLineLint(state, "string const &FileInfo::Pathname() const;", "")
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(state, "string Class<Type>::Method() const {\n" '  return "";\n' "}\n", "")
        self.TestSingleLineLint(
            state,
    ...
    def testNoSpacesInFunctionCalls(self, state):
        self.TestSingleLineLint(state, "TellStory(1, 3);", "")
        self.TestSingleLineLint(
            state,
            "TellStory(1, 3 );",
            "Extra space before )" "  [whitespace/parens] [2]",
        )
        self.TestSingleLineLint(state, "TellStory(1 /* wolf */, 3 /* pigs */);", "")
        self.TestMultiLineLint(
            state,
            """TellStory(1, 3
                                        );""",
    ...
    def testToDoComments(self, state: LintState):
        ...
        self.TestSingleLineLint(state, "//   TODOfix this", [start_space, missing_username, end_space])
        self.TestSingleLineLint(state, "//   TODO(ljenkins)fix this", [start_space, end_space])
        self.TestSingleLineLint(state, "//   TODO fix this", [start_space, missing_username])
        self.TestSingleLineLint(state, "// TODO fix this", missing_username)
        self.TestSingleLineLint(state, "// TODO: fix this", missing_username)
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(state, "// TODO(ljenkins):Fix this", end_space)
        self.TestSingleLineLint(state, "// TODO(ljenkins):", "")
        self.TestSingleLineLint(state, "// TODO(ljenkins): fix this", "")
        self.TestSingleLineLint(state, "// TODO(ljenkins): Fix this", "")
        self.TestSingleLineLint(state, "#if 1  // TEST_URLTODOCID_WHICH_HAS_THAT_WORD_IN_IT_H_", "")
        self.TestSingleLineLint(state, "// See also similar TODO above", "")
        self.TestSingleLineLint(
            state,
            r'EXPECT_EQ("\\", ' r'NormalizePath("/./../foo///bar/..//x/../..", ""));',
            "",
        )
    ...
    def testTwoSpacesBetweenCodeAndComments(self, state):
        ...
        self.TestSingleLineLint(state, "int i = 0;  // Having two spaces is fine.", "")
        self.TestSingleLineLint(state, "int i = 0;   // Having three spaces is OK.", "")
        self.TestSingleLineLint(state, "// Top level comment", "")
        self.TestSingleLineLint(state, "  // Line starts with two spaces.", "")
        ...
        self.TestSingleLineLint(state, 'printf("// In quotes.")', "")
        self.TestSingleLineLint(state, 'printf("\\"%s // In quotes.")', "")
        self.TestSingleLineLint(state, 'printf("%s", "// In quotes.")', "")

    def testSpaceAfterCommentMarker(self, state: LintState):
        self.TestSingleLineLint(state, "//", "")
        self.TestSingleLineLint(
            state,
            "//x",
            "Should have a space between // and comment" "  [whitespace/comments] [4]",
        )
        self.TestSingleLineLint(state, "// x", "")
        self.TestSingleLineLint(state, "///", "")
        self.TestSingleLineLint(state, "/// x", "")
        self.TestSingleLineLint(state, "//!", "")
        self.TestSingleLineLint(state, "//----", "")
        ...
        self.TestSingleLineLint(state, "//////", "")
        self.TestSingleLineLint(state, "////// x", "")
        self.TestSingleLineLint(state, "///< x", "")  # After-member Doxygen comment
        self.TestSingleLineLint(state, "//!< x", "")  # After-member Doxygen comment
        self.TestSingleLineLint(
            state,
            "////x",
    ...
    def testUsingLiteralsNamespaces(self, state: LintState):
        self.TestSingleLineLint(
            state,
    ...
    def testInvalidUtf8(self, state):
        def DoTest(self, raw_bytes, has_invalid_utf8):
            ...
            if sys.version_info < (3,):
                ...
                unidata = str(raw_bytes, "utf8", "replace").split("\n")
            cpplint.ProcessFileData(state, "foo.cc", "cc", unidata, error_collector)
            # The warning appears only once.
    ...
    def testBlankLinesCheck(self, state: LintState):
        self.TestBlankLinesCheck(state, ["{\n", "\n", "\n", "}\n"], 1, 1)
        self.TestBlankLinesCheck(state, ["  if (foo) {\n", "\n", "  }\n"], 1, 1)
        self.TestBlankLinesCheck(state, ["\n", "// {\n", "\n", "\n", "// Comment\n", "{\n", "}\n"], 0, 0)
        self.TestBlankLinesCheck(state, ["\n", 'run("{");\n', "\n"], 0, 0)
        self.TestBlankLinesCheck(state, ["\n", "  if (foo) { return 0; }\n", "\n"], 0, 0)
        self.TestBlankLinesCheck(state, ["int x(\n", "    int a) {\n", "\n", "return 0;\n", "}"], 0, 0)
        self.TestBlankLinesCheck(state, ["int x(\n", "    int a) const {\n", "\n", "return 0;\n", "}"], 0, 0)
        self.TestBlankLinesCheck(state, ["int x(\n", "     int a) {\n", "\n", "return 0;\n", "}"], 1, 0)
        self.TestBlankLinesCheck(state, ["int x(\n", "   int a) {\n", "\n", "return 0;\n", "}"], 1, 0)
    ...
    def testBlankLineBeforeSectionKeyword(self, state):
        ...
        cpplint.ProcessFileData(
            ...
            + ([""] * 100)  # warning 3
            + [  # Make A and B longer than 100 lines
                "  };",
                "  struct C {",
                "   protected:",
    ...
    def testElseOnSameLineAsClosingBraces(self, state):
        ...
        cpplint.ProcessFileData(
            ...
            "foo.cc",
            "cc",
            ["if (hoge) {", "", "}", "else", "{", "", "}"],  # Warning on this line
            error_collector,
        )
        assert 1 == error_collector.Results().count(
            "An else should appear on the same line as the preceding }  [whitespace/newline] [4]"
    ...
    def testLambdasOnSameLine(self, state):
        ...
        cpplint.ProcessFileData(
            ...
            "foo.cc",
            "cc",
            ["const auto result = std::any_of(vector.begin(), " "vector.end(), " "[](const int i) { return i > 0; });"],
            error_collector,
        )
        assert 0 == error_collector.Results().count("More than one command on the same line  [whitespace/newline] [0]")
        ...
        cpplint.ProcessFileData(
            ...
            "foo.cc",
            "cc",
            ["return mutex::Lock<void>([this]() { " "this->ReadLock(); }, [this]() { " "this->ReadUnlock(); });"],
            error_collector,
        )
        assert 0 == error_collector.Results().count("More than one command on the same line  [whitespace/newline] [0]")
    ...
    def testEndOfNamespaceComments(self, state):
        ...
        cpplint.ProcessFileData(
            ...
            + ([""] * 10)
            + ["}", "}", "namespace {"]  # Warning here  # Warning here
            + ([""] * 10)
            + ["}", "namespace {"]  # Warning here
            + ([""] * 10)
            + ["}  // namespace some description", "namespace {"]  # Anon warning
            + ([""] * 10)
            + ["}  // namespace anonymous", "namespace {"]  # Variant warning
            + ([""] * 10)
            + ["}  // anonymous namespace (utils)", "namespace {"]  # Variant
            + ([""] * 10)
            + ["}  // anonymous namespace", "namespace missing_comment {"]  # No warning
            + ([""] * 10)
            + ["}", "namespace no_warning {"]  # Warning here
            + ([""] * 10)
        ...
        assert 1 == error_collector.Results().count(
            'Namespace should be terminated with "// namespace expected"' "  [readability/namespace] [5]"
        )
        assert 1 == error_collector.Results().count(
            'Namespace should be terminated with "// namespace outer"' "  [readability/namespace] [5]"
        )
        assert 1 == error_collector.Results().count(
            'Namespace should be terminated with "// namespace nested"' "  [readability/namespace] [5]"
        )
        ...
        assert 1 == error_collector.Results().count(
            'Namespace should be terminated with "// namespace missing_comment"' "  [readability/namespace] [5]"
        )
        assert 0 == error_collector.Results().count(
            'Namespace should be terminated with "// namespace no_warning"' "  [readability/namespace] [5]"
        )

    def testElseClauseNotOnSameLineAsElse(self, state: LintState):
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(
            state,
            "  else ifDoSomethingElse();",
            "Else clause should never be on same line as else " "(use 2 lines)  [whitespace/newline] [4]",
        )
        self.TestSingleLineLint(state, "  } else if (blah) {", "")
        self.TestSingleLineLint(state, "  variable_ends_in_else = true;", "")

    def testComma(self, state: LintState):
        self.TestSingleLineLint(state, "a = f(1,2);", "Missing space after ,  [whitespace/comma] [3]")
        self.TestSingleLineLint(
            state,
            "int tmp=a,a=b,b=tmp;",
            [
        ...
        self.TestSingleLineLint(state, "f(a, /* name */ b);", "")
        self.TestSingleLineLint(state, "f(a, /* name */b);", "")
        self.TestSingleLineLint(state, "f(a, /* name */-1);", "")
        self.TestSingleLineLint(state, 'f(a, /* name */"1");', "")
        self.TestSingleLineLint(state, "f(1, /* empty macro arg */, 2)", "")
        self.TestSingleLineLint(state, "f(1,, 2)", "")
        self.TestSingleLineLint(state, "operator,()", "")
        self.TestSingleLineLint(state, "operator,(a,b)", "Missing space after ,  [whitespace/comma] [3]")

    def testEqualsOperatorSpacing(self, state):
        self.TestSingleLineLint(state, "int tmp= a;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(state, "int tmp =a;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(state, "int tmp=a;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(state, "int tmp= 7;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(state, "int tmp =7;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(state, "int tmp=7;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(state, "int* tmp=*p;", "Missing spaces around =  [whitespace/operators] [4]")
        self.TestSingleLineLint(
            state,
            "int* tmp= *p;",
            "Missing spaces around =  [whitespace/operators] [4]",
        ...
        self.TestSingleLineLint(state, "a&=42;", "")
        self.TestSingleLineLint(state, "a|=42;", "")
        self.TestSingleLineLint(state, "a^=42;", "")
        self.TestSingleLineLint(state, "a+=42;", "")
        self.TestSingleLineLint(state, "a*=42;", "")
        self.TestSingleLineLint(state, "a/=42;", "")
        self.TestSingleLineLint(state, "a%=42;", "")
        self.TestSingleLineLint(state, "a>>=5;", "")
        self.TestSingleLineLint(state, "a<<=5;", "")

    def testShiftOperatorSpacing(self, state: LintState):
        self.TestSingleLineLint(state, "a<<b", "Missing spaces around <<  [whitespace/operators] [3]")
        self.TestSingleLineLint(state, "a>>b", "Missing spaces around >>  [whitespace/operators] [3]")
        self.TestSingleLineLint(state, "1<<20", "")
        self.TestSingleLineLint(state, "1024>>10", "")
        self.TestSingleLineLint(state, "Kernel<<<1, 2>>>()", "")

    def testIndent(self, state):
        self.TestSingleLineLint(state, "static int noindent;", "")
        self.TestSingleLineLint(state, "  int two_space_indent;", "")
        self.TestSingleLineLint(state, "    int four_space_indent;", "")
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(state, " public:", "")
        self.TestSingleLineLint(state, "  protected:", "")
        self.TestSingleLineLint(state, "   private:", "")
        self.TestSingleLineLint(state, " protected: \\", "")
        self.TestSingleLineLint(state, "  public:      \\", "")
        self.TestSingleLineLint(state, "   private:   \\", "")
    ...
    def testTab(self, state: LintState):
        self.TestSingleLineLint(state, "\tint16 a;", "Tab found; better to use spaces  [whitespace/tab] [1]")
        self.TestSingleLineLint(
            state,
            "int16 a = 5;\t\t// set a to 5",
            "Tab found; better to use spaces  [whitespace/tab] [1]",
        )

    def test_parse_arguments(self, state):
        old_output_format = state.output_format
        old_verbose_level = state.verbose_level
        old_headers = state._hpp_headers
        old_filters = state.filters
        old_line_length = state._line_length
        old_valid_extensions = state._valid_extensions
        try:
            ...
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, [])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--badopt"])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--help"])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--version"])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--v=0"])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--filter="])
            # This is illegal because all filters must start with + or -
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--filter=foo"])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--filter=+a,b,-c"])
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--headers"])

            assert ["foo.cc"] == cli.parse_arguments(state, ["foo.cc"])
            assert old_output_format == state.output_format
            assert old_verbose_level == state.verbose_level

            assert ["foo.cc"] == cli.parse_arguments(state, ["--v=1", "foo.cc"])
            assert 1 == state.verbose_level
            assert ["foo.h"] == cli.parse_arguments(state, ["--v=3", "foo.h"])
            assert 3 == state.verbose_level
            assert ["foo.cpp"] == cli.parse_arguments(state, ["--verbose=5", "foo.cpp"])
            assert 5 == state.verbose_level
            with pytest.raises(ValueError):
                cli.parse_arguments(state, ["--v=f", "foo.cc"])

            assert ["foo.cc"] == cli.parse_arguments(state, ["--output=emacs", "foo.cc"])
            assert "emacs" == state.output_format
            assert ["foo.h"] == cli.parse_arguments(state, ["--output=vs7", "foo.h"])
            assert "vs7" == state.output_format
            with pytest.raises(SystemExit):
                cli.parse_arguments(state, ["--output=blah", "foo.cc"])

            filt = "-,+whitespace,-whitespace/indent"
            assert ["foo.h"] == cli.parse_arguments(state, ["--filter=" + filt, "foo.h"])
            assert [
                "-",
                "+whitespace",
                "-whitespace/indent",
            ...
            assert ["foo.cc", "foo.h"] == cli.parse_arguments(state, ["foo.cc", "foo.h"])

            state._hpp_headers = old_headers
            state._valid_extensions = old_valid_extensions
            assert ["foo.h"] == cli.parse_arguments(state, ["--linelength=120", "foo.h"])
            assert 120 == state._line_length
            assert set(["h", "hh", "hpp", "hxx", "h++", "cuh"]) == state.GetHeaderExtensions()

            state._hpp_headers = old_headers
            state._valid_extensions = old_valid_extensions
            assert ["foo.h"] == cli.parse_arguments(state, ["--headers=h", "foo.h"])
            assert set(["h", "c", "cc", "cpp", "cxx", "c++", "cu"]) == state.GetAllExtensions()

            state._hpp_headers = old_headers
            state._valid_extensions = old_valid_extensions
            assert ["foo.h"] == cli.parse_arguments(state, ["--extensions=hpp,cpp,cpp", "foo.h"])
            assert set(["hpp", "cpp"]) == state.GetAllExtensions()
            assert set(["hpp"]) == state.GetHeaderExtensions()

            state._hpp_headers = old_headers
            state._valid_extensions = old_valid_extensions
            assert ["foo.h"] == cli.parse_arguments(
                state,
                ["--extensions=cpp,cpp", "--headers=hpp,h", "foo.h"],
            )
            assert set(["hpp", "h"]) == state.GetHeaderExtensions()
            assert set(["hpp", "h", "cpp"]) == state.GetAllExtensions()

        finally:
            sys.stdout = sys.__stdout__
            sys.stderr = sys.__stderr__
            state.output_format = old_output_format
            state.verbose_level = old_verbose_level
            state.filters = old_filters
            state._line_length = old_line_length
            state._valid_extensions = old_valid_extensions
            state._hpp_headers = old_headers

    def testRecursiveArgument(self, state):
        ...
        try:
            ...
            cli._excludes = None
            actual = cli.parse_arguments(state, ["--recursive", "one.cpp", "src"])
            assert set(expected) == set(actual)
        finally:
            os.chdir(working_dir)
            shutil.rmtree(temp_dir)

    def testRecursiveExcludeInvalidFileExtension(self, state):
        working_dir = os.getcwd()
        temp_dir = os.path.realpath(tempfile.mkdtemp())
        try:
            src_dir = os.path.join(temp_dir, "src")
    ...
    def testRecursiveExclude(self, state):
        working_dir = os.getcwd()
        temp_dir = os.path.realpath(tempfile.mkdtemp())
        try:
            src_dir = os.path.join(temp_dir, "src")
            ...
            cli._excludes = None
            actual = cli.parse_arguments(state, ["src"])
            assert set(["src"]) == set(actual)

            cli._excludes = None
            actual = cli.parse_arguments(state, ["--recursive", "src"])
            assert set(expected) == set(actual)
            ...
            cli._excludes = None
            actual = cli.parse_arguments(state, ["--recursive", "--exclude=src", "."])
            assert expected == set(actual)
        finally:
            os.chdir(working_dir)
            shutil.rmtree(temp_dir)

    def testJUnitXML(self, state):
        try:
            state._junit_errors = []
            ...
            assert expected == state.FormatJUnitXML()

            state._junit_errors = ["ErrMsg1"]
            state._junit_failures = []
            expected = (
                '<?xml version="1.0" encoding="UTF-8" ?>\n'
            ...
            state._junit_errors = ["ErrMsg1", "ErrMsg2"]
            state._junit_failures = []
            expected = (
                '<?xml version="1.0" encoding="UTF-8" ?>\n'
            ...
            state._junit_errors = ["ErrMsg"]
            state._junit_failures = [("File", 5, "FailMsg", "category/subcategory", 3)]
            expected = (
                '<?xml version="1.0" encoding="UTF-8" ?>\n'
            ...
            state._junit_errors = ["&</error>"]
            state._junit_failures = [("File1", 5, "&</failure>", "category/subcategory", 3)]
            expected = (
                '<?xml version="1.0" encoding="UTF-8" ?>\n'
                '<testsuite errors="1" failures="1" name="cpplint" tests="2">'
    ...
    def testQuiet(self, state):
        assert state.quiet is False
        cli.parse_arguments(state, ["--quiet", "one.cpp"])
        assert state.quiet is True

    def testLineLength(self, state: LintState):
        old_line_length = state._line_length
        try:
            state._line_length = 80
            self.TestSingleLineLint(state, "// H %s" % ("H" * 75), "")
            self.TestSingleLineLint(
                state,
                "// H %s" % ("H" * 76),
                "Lines should be <= 80 characters long" "  [whitespace/line_length] [2]",
            )
            state._line_length = 120
            self.TestSingleLineLint(state, "// H %s" % ("H" * 115), "")
            self.TestSingleLineLint(
                state,
                "// H %s" % ("H" * 116),
    ...
    def testFilter(self, state: LintState):
        state.filters = ["-", "+whitespace", "-whitespace/indent"]
        self.TestSingleLineLint(
            state,
            "// Hello there ",
            "Line ends in whitespace.  Consider deleting these extra spaces." "  [whitespace/end_of_line] [4]",
        )
        self.TestSingleLineLint(state, "int a = (int)1.0;", "")
        self.TestSingleLineLint(state, " weird opening space", "")

    def testDefaultFilter(self, state_with_defaults: LintState):
        state_with_defaults.filters = ""
        assert "-build/include_alpha" in state_with_defaults.filters
    ...
    def testUnnamedNamespacesInHeaders(self, state: LintState):
        for extension in ["h", "hpp", "hxx", "h++", "cuh"]:
            self.doTestUnnamedNamespacesInHeaders(state, extension)

    def doTestUnnamedNamespacesInHeaders(self, state: LintState, extension):
        ...
        self.TestLanguageRulesCheck(state, "foo." + extension, "namespace {  \\", "")
        # named namespaces are OK.
        self.TestLanguageRulesCheck(state, "foo." + extension, "namespace foo {", "")
        self.TestLanguageRulesCheck(state, "foo." + extension, "namespace foonamespace {", "")

    def testUnnamedNamespacesInNonHeaders(self, state: LintState):
        for extension in ["c", "cc", "cpp", "cxx", "c++", "cu"]:
            self.TestLanguageRulesCheck(state, "foo." + extension, "namespace {", "")
            self.TestLanguageRulesCheck(state, "foo." + extension, "namespace foo {", "")
    ...
    def testBuildClass(self, state):
        ...
        self.TestMultiLineLint(
            state,
            "class Foo {",
            "Failed to find complete declaration of class Foo" "  [build/class] [5]",
        )
        # Do the same for namespaces
        self.TestMultiLineLint(
            state,
            "namespace Foo {",
            "Failed to find complete declaration of namespace Foo" "  [build/namespaces] [5]",
        )
        # Don't warn on forward declarations of various types.
        self.TestMultiLineLint(state, "class Foo;", "")
        self.TestMultiLineLint(
            state,
            """struct Foo*
             foo = NewFoo();""",
    ...
    def testBuildForwardDecl(self, state: LintState):
        # The crosstool compiler we currently use will fail to compile the
        # code in this test, so we might consider removing the lint check.
        self.TestSingleLineLint(
            state,
            "class Foo::Goo;",
            "Inner-style forward declarations are invalid." "  Remove this line." "  [build/forward_decl] [5]",
        )

    def GetBuildHeaderGuardPreprocessorSymbol(self, state: LintState, file_path):
    ...
    def testBuildHeaderGuard(self, state: LintState):
        ...
        cpplint.ProcessFileData(state, file_path, "h", [], error_collector)
        assert 1 == error_collector.ResultList().count(
            "No #ifndef header guard found, suggested CPP variable is: %s" "  [build/header_guard] [5]" % expected_guard
        ), error_collector.ResultList()
        ...
        # Wrong guard
        error_collector = ErrorCollector()
        cpplint.ProcessFileData(state, file_path, "h", ["#ifndef FOO_H", "#define FOO_H"], error_collector)
        assert 1 == error_collector.ResultList().count(
            "#ifndef header guard has wrong style, please use: %s" "  [build/header_guard] [5]" % expected_guard
        ), error_collector.ResultList()

        # No define
        error_collector = ErrorCollector()
        cpplint.ProcessFileData(state, file_path, "h", ["#ifndef %s" % expected_guard], error_collector)
        assert 1 == error_collector.ResultList().count(
            "No #ifndef header guard found, suggested CPP variable is: %s" "  [build/header_guard] [5]" % expected_guard
        ), error_collector.ResultList()
        ...
        cpplint.ProcessFileData(
            ...
            file_path,
            "h",
            ["#ifndef %s" % expected_guard, "#define FOO_H"],
            error_collector,
        )
        assert 1 == error_collector.ResultList().count(
            "No #ifndef header guard found, suggested CPP variable is: %s" "  [build/header_guard] [5]" % expected_guard
        ...
        try:
            ...
            assert 1 == error_collector.ResultList().count(
                "#ifndef header guard has wrong style, please use: %s" "  [build/header_guard] [0]" % expected_guard
            ), error_collector.ResultList()
        finally:
            state.verbose_level = old_verbose_level
    ...
    def testBuildHeaderGuardWithRoot(self, state):
        ...
        try:
            ...
            self.doTestBuildHeaderGuardWithRoot(state, header_directory)
        finally:
            shutil.rmtree(temp_directory)

    def doTestBuildHeaderGuardWithRoot(self, state, header_directory):
        ...
        # relative directory
        state._root = "cpplint"
        assert "CPPLINT_TEST_HEADER_H_" == cpplint.GetHeaderGuardCPPVariable(state, file_path)
        ...
        state._root = os.path.join("cpplint", "nested")
        actual = cpplint.GetHeaderGuardCPPVariable(state, nested_file_path)
        assert "CPPLINT_TEST_HEADER_H_" == actual

        # absolute directory
        # (note that CPPLINT.cfg root=setting is always made absolute)
        state._root = header_directory
        assert "CPPLINT_TEST_HEADER_H_" == cpplint.GetHeaderGuardCPPVariable(state, file_path)

        state._root = nested_header_directory
        assert "CPPLINT_TEST_HEADER_H_" == cpplint.GetHeaderGuardCPPVariable(state, nested_file_path)

        # --root flag is ignored if an non-existent directory is specified.
        state._root = "NON_EXISTENT_DIR"
        assert "CPPLINT_CPPLINT_TEST_HEADER_H_" == cpplint.GetHeaderGuardCPPVariable(state, file_path)
        ...
        state._root = styleguide_parent_path
        ...
        assert "%sCPPLINT_CPPLINT_TEST_HEADER_H_" % expected_prefix == cpplint.GetHeaderGuardCPPVariable(
            state, file_path
        )
        ...
        # '..'
        state._root = styleguide_rel_path
        assert "CPPLINT_CPPLINT_TEST_HEADER_H_" == cpplint.GetHeaderGuardCPPVariable(state, file_path)
    ...
    def testIncludeItsHeader(self, state):
        ...
        try:
            ...
            error_collector = ErrorCollector()
            cpplint.ProcessFileData(state, "test/foo.cc", "cc", [""], error_collector)
            ...
            expected = (
                "{dir}/{fn}.cc should include its header file {dir}/{fn}.h{unix_text}  [build/include] [5]".format(
                    fn="foo",
                    dir=test_directory,
            ...
            error_collector = ErrorCollector()
            cpplint.ProcessFileData(state, "test/Bar.cc", "cc", [r'#include "Bar.h"', ""], error_collector)
            assert 0 == error_collector.Results().count(expected)
    ...
    def testBuildHeaderGuardWithRepository(self, state):
        ...
        try:
            ...
            state._repository = None
            state._root = None

    def testBuildInclude(self, state: LintState):
        ...
        self.TestSingleLineLint(state, '#include "baz.aa"', "")
        self.TestSingleLineLint(state, '#include "dir/foo.h"', "")
        self.TestSingleLineLint(state, '#include "Python.h"', "")
        self.TestSingleLineLint(state, '#include "lua.h"', "")

    def testHppInclude(self, state):
        code = "\n".join(["#include <vector>", "#include <boost/any.hpp>"])
        self.TestLanguageRulesCheck(state, "foo.h", code, "")
    ...
    def testBuildPrintfFormat(self, state):
        ...
        assert 4 == error_collector.Results().count(
            "%, [, (, and { are undefined character escapes.  Unescape them." "  [build/printf_format] [3]"
        )
    ...
    def testRuntimePrintfFormat(self, state):
        self.TestSingleLineLint(
            state,
        ...
        self.TestSingleLineLint(
            ...
            "%N$ formats are unconventional.  Try rewriting to avoid them." "  [runtime/printf_format] [2]",
        )
    ...
    def testBuildStorageClass(self, state: LintState):
        ...
        self.TestSingleLineLint(state, "const int static foo = 5;", build_storage_class_error_message)

        self.TestSingleLineLint(state, "char static foo;", build_storage_class_error_message)

        self.TestSingleLineLint(state, "double const static foo = 2.0;", build_storage_class_error_message)
        ...
        self.TestSingleLineLint(state, "int register foo = 0;", build_storage_class_error_message)
        ...
        for unused_i in range(10):
            ...
            decl_specs = other_decl_specs[0:insertion_point] + [storage_class] + other_decl_specs[insertion_point:]

            self.TestSingleLineLintLogCodeOnError(state, " ".join(decl_specs) + ";", build_storage_class_error_message)

            # but no error if storage class is first
            self.TestSingleLineLintLogCodeOnError(state, storage_class + " " + " ".join(other_decl_specs), "")
    ...
    def testInvalidIncrement(self, state: LintState):
        self.TestSingleLineLint(
            state,
            "*count++;",
            "Changing pointer instead of value (or unused value of " "operator*).  [runtime/invalid_increment] [5]",
        )

    def testSnprintfSize(self, state: LintState):
        self.TestSingleLineLint(state, "vsnprintf(NULL, 0, format)", "")
        self.TestSingleLineLint(
            state,
            "snprintf(fisk, 1, format)",
            "If you can, use sizeof(fisk) instead of 1 as the 2nd arg " "to snprintf.  [runtime/printf] [3]",
        )
