...
from .lintstate import LintState, ParseNolintSuppressions
...
def CheckStyle(
    ...
    check_line_lengths(state, clean_lines, line_num)
    check_single_statement_on_each_line(state, clean_lines, line_num)
    CheckBraces(state, clean_lines, line_num)
    CheckTrailingSemicolon(state, clean_lines, line_num)
    CheckEmptyBlockBody(state, clean_lines, line_num)
    CheckSpacing(state, clean_lines, line_num, nesting_state)
    CheckOperatorSpacing(state, clean_lines, line_num)
    CheckParenthesisSpacing(state, clean_lines, line_num)
    CheckCommaSpacing(state, clean_lines, line_num)
    CheckBracesSpacing(state, clean_lines, line_num, nesting_state)
    CheckSpacingForFunctionCall(state, clean_lines, line_num)
    CheckCheck(state, clean_lines, line_num)
    CheckAltTokens(state, clean_lines, line_num)
    classinfo = nesting_state.innermost_class()
    if classinfo:
        CheckSectionSpacing(state, clean_lines, classinfo, line_num)


def check_line_lengths(state: LintState, clean_lines: CleansedLines, line_num: int):
    line = clean_lines.lines_without_raw_strings[line_num]
    # Check if the line is a header guard.
    is_header_guard = False
    file_extension = pathlib.Path(clean_lines.file_name).suffix[1:]  # we don't want the dot
    if state.IsHeaderExtension(file_extension):
        cppvar = GetHeaderGuardCPPVariable(state, clean_lines.file_name)
        if (
...
def check_single_statement_on_each_line(state: LintState, clean_lines: CleansedLines, line_num: int):
...
def CheckBraces(state: LintState, clean_lines, line_num):
...
def CheckTrailingSemicolon(state, clean_lines, line_num):
    ...
    if match:
        ...
        opening_parenthesis = ReverseCloseExpression(clean_lines, line_num, closing_brace_pos)
    ...
    if match:
        (endline, endlinenum, endpos) = CloseExpression(clean_lines, line_num, len(match.group(1)))
        if endpos > -1 and Match(r"^\s*;", endline[endpos:]):
            ...
            ParseNolintSuppressions(state, clean_lines.file_name, raw_lines[endlinenum - 1], endlinenum - 1)
            ParseNolintSuppressions(state, clean_lines.file_name, raw_lines[endlinenum], endlinenum)
...
def CheckEmptyBlockBody(state, clean_lines, line_num):
    ...
    if matched:
        ...
        (end_line, end_linenum, end_pos) = CloseExpression(clean_lines, line_num, line.find("("))
...
def CheckSpacing(state, clean_lines, line_num, nesting_state):
    ...
    if IsBlankLine(line) and not nesting_state.in_namespace_body() and not nesting_state.in_extern_c():
        ...
        #     // Something else
        #   }
        if line_num + 1 < clean_lines.num_lines():
            next_line = raw[line_num + 1]
    ...
    # TODO: checks should either only call other checks, or not call other checks.
    CheckComment(state, line, clean_lines.file_name, line_num, next_line_start)
    ...
    if Search(r"\w\s+\[(?!\[)", line) and not Search(r"(?:auto&?|delete|return)\s+\[", line):
        state.log_error(clean_lines.file_name, line_num, "whitespace/braces", 5, "Extra space before [")
...
def CheckComment(state, line, filename, line_num, next_line_start):
    ...
    if comment_position != -1:
        ...
        if re.sub(r"\\.", "", line[0:comment_position]).count('"') % 2 == 0:
            # Allow one space for new scopes, two spaces otherwise:
            if not (Match(r"^.*{ *//", line) and next_line_start == comment_position) and (
                (comment_position >= 1 and line[comment_position - 1] not in string.whitespace)
                or (comment_position >= 2 and line[comment_position - 2] not in string.whitespace)
            ):
                state.log_error(
                    filename,
                    line_num,
...
def CheckOperatorSpacing(state, clean_lines, line_num):
    ...
    if match:
        ...
        if match:
            (_, _, end_pos) = CloseExpression(clean_lines, line_num, len(match.group(1)))
            if end_pos <= -1:
                state.log_error(
                    clean_lines.file_name,
                    line_num,
        ...
        match = Match(r"^(.*[^-\s>])>[^\s=>,]", line)
        if match:
            (_, _, start_pos) = ReverseCloseExpression(clean_lines, line_num, len(match.group(1)))
            if start_pos <= -1:
                state.log_error(
                    clean_lines.file_name,
                    line_num,
...
def CheckParenthesisSpacing(state, clean_lines, line_num):
...
def CheckCommaSpacing(state: LintState, clean_lines: CleansedLines, line_num: int):
    ...
    # elided comments.
    if Search(r",[^,\s]", ReplaceAll(r"\boperator\s*,\s*\(", "F(", line)) and Search(r",[^,\s]", raw[line_num]):
        state.log_error(clean_lines.file_name, line_num, "whitespace/comma", 3, "Missing space after ,")
    ...
    # space after ;
    if Search(r";[^\s};\\)/]", line):
        state.log_error(clean_lines.file_name, line_num, "whitespace/semicolon", 3, "Missing space after ;")


def CheckBracesSpacing(state, clean_lines, line_num, nesting_state):
...
def CheckSpacingForFunctionCall(state, clean_lines, line_num):
    ...
    if (  # Ignore control structures.
        ...
        if Search(r"\w\s*\(\s(?!\s*\\$)", fncall):  # a ( used for a fn call
            ...
            state.log_error(clean_lines.file_name, line_num, "whitespace/parens", 2, "Extra space after (")
        if (
            Search(r"\w\s+\(", fncall)
...
def CheckCheck(state, clean_lines, line_num):
    ...
    (check_macro, start_pos) = FindCheckMacro(lines[line_num])
    if not check_macro:
        return

    # Find end of the boolean expression by matching parentheses
    (last_line, end_line, end_pos) = CloseExpression(clean_lines, line_num, start_pos)
    if end_pos < 0:
        return
    ...
    if line_num == end_line:
        expression = lines[line_num][start_pos + 1: end_pos - 1]
    else:
        expression = lines[line_num][start_pos + 1:]
...
def CheckAltTokens(state, clean_lines, line_num):
...
def CheckSectionSpacing(state, clean_lines, class_info, line_num):
    ...
    if class_info.last_line - class_info.starting_line_num <= 24 or line_num <= class_info.starting_line_num:
        return

    matched = Match(r"\s*(public|protected|private):", clean_lines.lines[line_num])
    if matched:
        ...
        prev_line = clean_lines.lines[line_num - 1]
        if (
            ...
            for i in range(class_info.starting_line_num, line_num):
                if Search(r"\{\s*$", clean_lines.lines[i]):
                    end_class_head = i
                    break
            if end_class_head < line_num - 1:
                state.log_error(
                    clean_lines.file_name,
                    line_num,
                    "whitespace/blank_line",
